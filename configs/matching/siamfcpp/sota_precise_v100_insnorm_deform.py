# stage 2
exp_name = 'sar_opt_precise_v1_insnorm_deform'
dataroot = "/work_base/datasets/stage2"
lr = 0.6 * 1e-3
weight_decay = 2e-6
offset_lr_mult = 0.1
############################################
...
model = dict(
    ...
    generator=dict(
        type='SIAMFCPP_two_insnorm_deform', # SIAMFCPP_P
        in_cha=1,
        channels=48,
        stacked_convs = 2,
        feat_channels = 48,
...
optimizers = dict(generator=dict(type='AdamW', lr=lr, betas=(0.9, 0.999), weight_decay=weight_decay, 
                                paramwise_cfg=dict(custom_keys={'offset': dict(lr_mult=offset_lr_mult)})))
...
load_from = f'./workdirs/{exp_name}/20210509_160853/checkpoints/epoch_410' 
resume_from = None
resume_optim = True
workflow = 'train'
