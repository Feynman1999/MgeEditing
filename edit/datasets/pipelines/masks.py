...
class GenerateRandomMasks(object):
    ...
    def __call__(self, results):
        """
            在results中加入masks
        """
        length = results['max_frame_num']
        results['masks'] = create_random_shape_with_random_motion(length, imageHeight=self.h, imageWidth=self.w)
        results['masks'] = [ np.array(mask) for mask in results['masks']] # [240, 432]

        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f', h={self.h}, w={self.w}'
        return repr_str
