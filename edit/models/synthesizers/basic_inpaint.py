...
from .basicinpaint import train_batch, test_batch
from tensorboardX import SummaryWriter

epoch_dict = {}

def adjust_learning_rate(optimizer, epoch):
    if epoch>=200 and epoch % 10 == 0  and epoch_dict.get(epoch, None) is None:
...
class basicinpaint_synthesizer(BaseModel):
    ...
    def __init__(self, generator, discriminator, pixel_loss, adv_loss, loss_weight,
        ...
        super(basicinpaint_synthesizer, self).__init__()
    ...
    def solve(self, start, len, save_path):
        """
            从index为start开始取，长度为len
        """
        img = np.stack(self.img_list[start:start + len], axis=1) # [1,len,c,h,w]
