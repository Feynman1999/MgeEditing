...
from .utils import get_bilinear, cal_flow_mask
from tqdm import tqdm

# 最两边的hidden直接使用mask为0即可
def flowmask_train_generator_batch(image, label, *, gm, netG, netloss):
    ...
    with gm:
        ...
        # solve masks
        mask_1_0 = cal_flow_mask(flows_1_0, flows_0_1, thredhold=3)
        mask_0_1 = cal_flow_mask(flows_0_1, flows_1_0, thredhold=3)
        for layer_idx in range(netG.RNN_layers):
            forward_hiddens = []
            backward_hiddens = []
            # 对于左边第一帧和右边最后一帧，直接传入hidden为0, mask为0, flow也为0
            hidden = F.zeros((2*B, netG.hidden_channels, h, w))
            mask = F.zeros((2*B, 1, h, w))
            flow = F.zeros((2*B, 2, h, w))
            hidden = netG(F.concat([now_hiddens[0], now_hiddens[T-1]], axis=0), hidden, flow, layer_idx, mask)
            ...
            for i in range(T-1):
                ...
                f_mask = mask_1_0[i*B:(i+1)*B, ...]
                b_mask = mask_0_1[(T-i-2)*B:(T-i-1)*B, ...]
                flow = F.concat([f_flow, b_flow], axis=0) # [2B,2,H,W]
                mask = F.concat([f_mask, b_mask], axis=0) # [2B,1,H,W]
                hidden = netG(F.concat([now_hiddens[i+1], now_hiddens[T-i-2]], axis=0), hidden, flow, layer_idx, mask)
                forward_hiddens.append(hidden[0:B, ...])
                backward_hiddens.append(hidden[B:2*B, ...])
            # update now_hiddens
            for i in range(T):
...
def flowmask_test_generator_batch(image, *, netG):
