...
def get_mid_bicubic(image):
    mid_bicubic = image[:, 3, ...] # [B,C,H,W]
...
@trace(symbolic=True)
def train_generator_batch(image, mid_bicubic, label, *, opt, netG, netloss):
    netG.train()
...
class ManytoOneRestorer_v2(BaseModel):
    ...
    def train_step(self, batchdata):
        ...
        loss = train_generator_batch(data, mid_bicubic, label, opt=self.optimizers['generator'], netG=self.generator, netloss=self.pixel_loss)
    ...
    def test_step(self, batchdata, **kwargs):
        ...
        output = test_generator_batch(images, get_mid_bicubic(images), netG = self.generator)  # HR [B,C,4H,4W]
