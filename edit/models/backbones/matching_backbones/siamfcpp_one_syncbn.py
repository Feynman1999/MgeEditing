...
class SIAMFCPP_one_syncbn(M.Module):
    def __init__(self, in_cha,
        ...
        super(SIAMFCPP_one_syncbn, self).__init__()
        self.in_cha = in_cha
    ...
    def _init_feature_adjust(self):
        self.r_z_k = M.Sequential(
                ...
                M.SyncBatchNorm(self.feat_channels)
            )
    ...
    def _init_predictor(self):
        ...
        self.conv_cls = M.Sequential(
            M.Conv2d(self.feat_channels, self.cls_out_channels, kernel_size=1, stride=1, padding=0),
            M.SyncBatchNorm(self.cls_out_channels)
        )
        self.conv_reg = M.Sequential(
            M.Conv2d(self.feat_channels, 2, kernel_size=1, stride=1, padding=0),
            M.SyncBatchNorm(2)
        )
...
class AlexNet_stride4(M.Module):
    def __init__(self, in_cha, ch=48):
        ...
        self.conv1 = M.Sequential(
                M.Conv2d(in_cha, ch//2, 11, 2, 5),
                M.SyncBatchNorm(ch//2),
                M.ReLU()
            )
        self.conv2 = M.Sequential(
                M.Conv2d(ch//2, ch, 5, 1, 2),
                M.SyncBatchNorm(ch),
                M.ReLU()
            )
        self.conv3 = M.Sequential(
                M.Conv2d(ch, ch, 3, 2, 1),
                M.SyncBatchNorm(ch),
                M.ReLU()
            )
