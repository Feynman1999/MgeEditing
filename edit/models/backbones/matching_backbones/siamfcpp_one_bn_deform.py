...
class SIAMFCPP_one_bn_deform(M.Module):
    def __init__(self, in_cha,
        ...
        super(SIAMFCPP_one_bn_deform, self).__init__()
        self.in_cha = in_cha
    ...
    def _init_feature_adjust(self):
        self.r_z_k = M.Sequential(
                ...
                M.BatchNorm2d(self.feat_channels),
            )
...
class AlexNet_stride4(M.Module):
    def __init__(self, in_cha, ch=48):
        ...
        self.offset2 = M.Sequential(
            ...
            M.Conv2d(ch, 3 * self.groups * 9, 3, 1, 1) # mask + offset
        )

        self.conv2 = M.DeformableConv2d(ch, ch, 3, 1, 1, groups=self.groups)
    ...
    def forward(self, x):
        ...
        if self.iter % 1000 == 0:
            print("offset2: ", F.mean(F.abs(offset2)))
            print("offset4: ", F.mean(F.abs(offset4)))
            print("mask2: ", F.mean(mask2))
            print("mask4: ", F.mean(mask4))
