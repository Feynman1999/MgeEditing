...
class SIAMFCPP_one_disweight(M.Module):
    def __init__(self, in_cha,
        ...
        super(SIAMFCPP_one_disweight, self).__init__()
        self.in_cha = in_cha
    ...
    def get_cls_reg_ctr_targets(self, points, gt_bboxes, bbox_scale = 0.1):
        ...
        # cls_labels
        gap = (gt_bboxes[:, 2, ...] - gt_bboxes[:, 0, ...] + 1) * (1-bbox_scale) / 2  - 0.01
    ...
    def get_batch_weight(self, cls_score, offsets, label):
        B, _, H, W = cls_score.shape
        cls_score = cls_score.reshape(B, -1)

        pred_box = self.get_box(self.fm_ctr, offsets)  # (B,4,H,W)
        output = []
        ...
        dis = F.norm(F.floor(output[:, 0:2] + 0.5) - label[:, 0:2], ord = 2, axis = 1, keepdims=True)  # (B, 1)

        return F.expand_dims(dis, axis=2) # (B,1,1)
    ...
    def loss(self,
        ...
        """
        batch_weight = self.get_batch_weight(cls_scores, bbox_preds, gt_bboxes)
        ...
        loss_cls = self.loss_cls(cls_scores, cls_labels.reshape(B, -1), batch_weight) / (B*H*W)
        ...
        reg_weight = (cls_labels.reshape(B, H*W, 1) * batch_weight).reshape((B*H*W, ))
        loss_reg = self.loss_bbox(bbox_preds, bbox_targets, weight = reg_weight) / cls_labels.sum()
...
class AlexNet_stride4(M.Module):
    def __init__(self, in_cha, ch=48):
        ...
        self.conv3 = M.conv_bn.ConvBnRelu2d(ch, ch, 3, 2, 1) # 2*48*9*200*200*48       1658880000
