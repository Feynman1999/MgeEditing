...
#             M.Conv2d(self.channels, self.feat_channels, kernel_size=3, padding=1, stride=1),
...
#             #                track_running_stats=True)
#             M.Sequential(
#                 M.Conv2d(self.feat_channels, self.feat_channels, kernel_size=3, padding=1, stride=1),
#                 GroupNorm(num_groups=4, num_channels=self.feat_channels),
#                 M.LeakyReLU()
#             )
#         )
...
#         # self.conv1 = M.conv_bn.ConvBnRelu2d(in_cha, ch // 2, kernel_size=3, stride=1, padding=1) #original
#         self.conv1 = M.Sequential(
#             M.Conv2d(in_cha, ch // 2, kernel_size=3, padding=1, stride=1),
#             GroupNorm(num_groups=2, num_channels=ch // 2),
#             M.LeakyReLU()
#         )
#         self.conv2 = CARBBlock(ch // 2)  #original
#         self.conv3 = M.Sequential(
#             M.Conv2d(ch // 2, ch, kernel_size=1, padding=0, stride=1),
...
#         self.conv1 = M.Sequential(
#             M.Conv2d(channel_num, channel_num, kernel_size=3, padding=1, stride=1),
#             GroupNorm(num_groups=4, num_channels=channel_num),
#             M.LeakyReLU(),
#             # M.conv_bn.ConvBnRelu2d(channel_num, channel_num, kernel_size=3, stride=1, padding=1),
#             M.Conv2d(channel_num, channel_num, kernel_size=3, padding=1, stride=1),
#         )
...
#             M.Conv2d(channel_num * 2, channel_num, kernel_size=1, padding=0, stride=1),
#             GroupNorm(num_groups=4, num_channels=channel_num),
#             M.LeakyReLU()
#         )
#         # self.conv2 = M.conv_bn.ConvBnRelu2d(channel_num * 2, channel_num, kernel_size=1, stride=1, padding=0)
...
#     def forward(self, x):
#         x1 = self.conv1(x)  # [B, C, H, W]
#         w = F.mean(x1, axis=[2,3], keepdims=False)  # [B,C]
#         w = self.linear(w)
#         w = F.expand_dims(w, axis=[2,3])
#         x1 = F.concat((x1, F.mul(x1, w)), axis=1)  # [B, 2C, H, W]
#         del w
