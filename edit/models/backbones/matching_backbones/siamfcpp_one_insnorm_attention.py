"""
在互相关前加入了non local attention
耗时巨大
参考cvpr2020 https://arxiv.org/abs/2004.06711
"""
import numpy as np
...
from edit.models.common import PAM_Module, CAM_Calculate, CAM_Use
...
class SIAMFCPP_one_insnorm_attention(M.Module):
    def __init__(self, in_cha,
        ...
        super(SIAMFCPP_one_insnorm_attention, self).__init__()
        self.in_cha = in_cha
    ...
    def _init_layers(self):
        ...
        self.feature_enhance = FeatureEnhance(self.channels, self.channels)
    ...
    def forward(self, sar, optical):
        ...
        # 对feat1 和 feat2做attention
        feat1, feat2 = self.feature_enhance(feat1, feat2)
...
class FeatureEnhance(M.Module):
    def __init__(self,
        ...
        self.conv_sar = M.Sequential(
            M.Conv2d(in_channels, out_channels, 3, 1, 1),
            InstanceNorm(out_channels),
            M.ReLU()
        )
        self.conv_opt = M.Sequential(
            M.Conv2d(in_channels, out_channels, 3, 1, 1),
            InstanceNorm(out_channels),
            M.ReLU()
        )

        self.pams_sar = PAM_Module(self.in_channels)
        self.pams_opt = PAM_Module(self.in_channels)
        self.cam_cals = CAM_Calculate(self.in_channels)
        self.cam_uses_1 = CAM_Use(self.in_channels)
        self.cam_uses_2 = CAM_Use(self.in_channels)
        self.cam_uses_3 = CAM_Use(self.in_channels)
        self.cam_uses_4 = CAM_Use(self.in_channels)

        self.initweights()

    def forward(self, z_f, x_f):
        z_sp_feat = self.pams_sar(z_f)
        x_sp_feat = self.pams_opt(x_f)

        z_attention = self.cam_cals(z_f)
        x_attention = self.cam_cals(x_f)

        zz_sc_feat = self.cam_uses_1(z_f, z_attention)
        xx_sc_feat = self.cam_uses_2(x_f, x_attention)

        zx_sc_feat = self.cam_uses_3(z_f, x_attention)
        xz_sc_feat = self.cam_uses_4(x_f, z_attention)

        z_f = z_sp_feat + zz_sc_feat + zx_sc_feat
        x_f = x_sp_feat + xx_sc_feat + xz_sc_feat

        z_f = self.conv_sar(z_f)
        x_f = self.conv_opt(x_f)

        return z_f, x_f

    def initweights(self):
        default_init_weights(self.conv_sar)
        default_init_weights(self.conv_opt)
