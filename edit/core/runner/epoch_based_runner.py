    ...
    def run(self, data_loaders, workflow, max_epochs):
        ...
        Args:
            ...
            workflow (list[tuple]): A list of (phase, epochs) to specify the
                running order and epochs. E.g, [('train', 10), ('test', 1)] means
                running 10 epochs for training and 1 epoch for test, iteratively.
        ...
        for i, flow in enumerate(workflow):
            ...
            if mode == 'train':
                ...
                self._iter = self.epoch * len(data_loaders[i])
                self.logger.info("{} iters for one epoch, trained iters: {}, total iters: {}".format(len(data_loaders[i]), self._iter, self._max_iters))
                # set the epoch in the dist sampler, so that the data is consitent
                # data_loaders[i].sampler.epoch = self.epoch , do not like torch and paddle, it's index are generated by np.random.RandomState(seed)
                break

        self.logger.info("Start running, work_dir: {}, workflow: {}, max epochs for train: {}".format(self.work_dir, workflow, max_epochs))
